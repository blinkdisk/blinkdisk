...
A differential backup starts with a complete [full backup](/glossary/what-is-a-full-backup) as its foundation. Every subsequent differential backup then copies all files that have changed since that initial full backup, not just since the last backup.

Think of it like adding layers to a stack. Your full backup is the base layer. Each differential backup adds another layer containing everything new or modified since the base. Unlike [incremental backups](/glossary/what-is-an-incremental-backup), each differential layer grows larger as more files change over time.
...
**Cloud Backup Scenarios**: When uploading to [cloud storage](/glossary/what-is-cloud-backup), differential backups reduce bandwidth usage compared to full backups while maintaining simpler restore procedures.
...
**Encrypt Your Data**: Always [encrypt](/glossary/what-is-end-to-end-encryption-in-backups) backups containing sensitive information, especially when storing them [offsite](/glossary/what-is-an-offsite-backup) or in the cloud.

**Maintain [Retention](/glossary/what-is-retention-in-backups)**: Keep multiple full backup sets with their associated differential backups. This protects against corruption in any single backup file.
