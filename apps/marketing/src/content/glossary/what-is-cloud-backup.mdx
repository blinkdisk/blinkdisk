...
Once the initial backup completes, the software monitors your files for changes. When you create, modify, or delete files, the backup updates automatically. Most services use [incremental backups](/glossary/what-is-an-incremental-backup) after the first upload, meaning only changed portions of files transfer, saving time and bandwidth.
...
Cloud backup creates immutable copies that remain unchanged regardless of what happens to your original files. Delete something by mistake? Your backup still has it. [Ransomware](/glossary/what-is-ransomware) strikes? Restore clean copies from before the attack.
...
Some services, including BlinkDisk, use zero-knowledge [encryption](/glossary/what-is-end-to-end-encryption-in-backups). This means your data gets encrypted on your device before transmission, and only you hold the decryption keys. The service stores your files but cannot read them. Maximum privacy comes with responsibility. Lose your password and your data becomes unrecoverable.

**Retention Policies**

Check how long the service keeps deleted files and previous versions. Some keep versions for 30 days, others for years. Longer [retention](/glossary/what-is-retention-in-backups) provides better protection against delayed discovery of problems.
...
Reputable backup services provide export options allowing you to download all your data. Additionally, the [3-2-1 backup rule](/glossary/what-is-the-3-2-1-backup-rule) (three copies on two different media with one offsite) means cloud backup supplements rather than replaces other backup methods.
...
Initial backups take time, especially for large collections. However, modern backup software uses [compression](/glossary/what-is-compression-in-backups) and block-level [deduplication](/glossary/what-is-deduplication-in-backups) to minimize upload sizes. After the first backup, only changes upload, typically just megabytes per day for average users.
