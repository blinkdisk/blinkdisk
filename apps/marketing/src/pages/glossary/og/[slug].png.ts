...
export const getStaticPaths: GetStaticPaths = async () => {
  ...
  return terms.map((term) => ({
    ...
    props: { question: term.data.question, shortAnswer: term.data.shortAnswer },
  }));
};

export const GET: APIRoute = async ({ props }) => {
  const { question, shortAnswer } = props as { question: string; shortAnswer: string };

  const truncatedAnswer = shortAnswer.length > 180 ? shortAnswer.slice(0, 177) + "..." : shortAnswer;
  const titleSize = question.length > 50 ? 52 : 60;
