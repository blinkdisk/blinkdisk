  ...
  return (
    <AlertDialogPortal>
      ...
      <AlertDialogPrimitive.Popup
        ...
        data-size={size}
        className={cn(
          "data-open:animate-in data-closed:animate-out data-closed:fade-out-0 data-open:fade-in-0 data-closed:zoom-out-95 data-open:zoom-in-95 bg-background ring-foreground/10 group/alert-dialog-content fixed left-1/2 top-1/2 z-50 grid w-full -translate-x-1/2 -translate-y-1/2 gap-4 rounded-xl p-4 outline-none ring-1 duration-100 data-[size=default]:max-w-xs data-[size=sm]:max-w-xs data-[size=default]:sm:max-w-sm",
          className,
        )}
        {...props}
      />
...
function AlertDialogDescription({
  ...
  return (
    <AlertDialogPrimitive.Description
      ...
      className={cn(
        "text-muted-foreground *:[a]:hover:text-foreground *:[a]:underline *:[a]:underline-offset-3 text-balance text-sm md:text-pretty",
        className,
      )}
      {...props}
    />
